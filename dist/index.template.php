...
function template_init()
{
	...
	// Lets define the default color palette, in order to leverage Color Changer mod functions
	$settings['color_palettes'] = array(
		'default' => array(
			'background' => '#fcdfa3',
			'blocks_alternate_color' => '#ebbf85',
			'blocks_color' => '#f7d38a',
			'borders_color' => '#6d386c',
			'buttons_bg' => '#edcfad',
			'buttons_border' => '#b7b7b7',
			'buttons_text_color' => '#595959',
			'content_target_bg' => '#f7f3ac',
			'footer' => '#dc915f',
			'foreground' => '#633896',
			'gradient_end' => '#ebbf85',
			'gradient_start' => '#ebbf85',
			'links' => '#334466',
			'locked_topic' => '#d0d6df',
	...
	// Load the new language string...
	loadLanguage('ColorChangerExt');

	// This bypasses the CC palette display/logic
	addJavaScriptVar('txt_cc_palettes', false);

	// Tell CC how to build our CSS - we use lotsa vars in this theme
	$settings['color_changes'] = array(
		'background' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_background')
			)
		),
		'foreground' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_foreground')
			)
		),
		'primary_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_primary_color')
			)
		),
		'secondary_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_secondary_color')
			)
		),
		'links' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_links')
			)
		),
		'gradient_end' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_gradient_end')
			)
		),
		'gradient_start' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_gradient_start')
			)
		),
		'blocks_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_blocks_color')
			)
		),
		'blocks_alternate_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_blocks_alternate_color')
			)
		),
		'content_target_bg' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_content_target_bg')
			)
		),
		'borders_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_borders_color')
			)
		),
		'top_section' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_top_section')
			)
		),
		'footer' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_footer')
			)
		),
		'buttons_text_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_buttons_text_color')
			)
		),
		'buttons_bg' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_buttons_bg')
			)
		),
		'buttons_border' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_buttons_border')
			)
		),
		'special_titles_color' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_special_titles_color')
			)
		),
		'sticky_topic' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_sticky_topic')
			)
		),
		'sticky_locked_topic' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_sticky_locked_topic')
			)
		),
		'unapproved_topic' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_unapproved_topic')
			)
		),
		'locked_topic' => array(
			array(
				'elements' => ':root',
				'properties' => array('--cc_locked_topic')
			)
		),
	...
	// Add some js variables for use by theme.js
	add_integration_function('integrate_load_theme', 'add_bsc_theme_jsvars', false, '$themedir/bsc_hooks.php');

	// Interestingly, if colors match the defaults, CC doesn't spit out inline CSS.  Normally it doesn't need to.
	// We need it spit out 100% of the time for this approach to work, so do so here...
	// (This is a (working) mess on the Current Theme screen as it is invoked multiple times, but OK everywhere else...)
	// It picks up the shadow setting just fine, so no need to do that here.  Just the colors in :root.
	$inline_css = ':root {';
	foreach($settings['color_changes'] AS $setting => $attr)
		if ($attr[0]['elements'] === ':root')
			$inline_css .= $attr[0]['properties'][0] . ': ' . $settings['cc_' . $setting] . ';';
	$inline_css .= '}';
	addInlineCss($inline_css);
}
...
{
	global $context, $scripturl, $txt, $modSettings, $mbname;
	...
	if (!empty($context['options']['bsc_disable_animations']))
		addJavaScriptVar('bsc_disable_anim_flag', true);
	else
		addJavaScriptVar('bsc_disable_anim_flag', false);
...
function template_body_above()
{
	...
	// Use our default svg header if a logo wasn't provided
	echo '
	<div id="header">
		<h1 class="forumtitle">
			<a id="top" href="', $scripturl, '" style="display: block;">', empty($context['header_logo_url_html_safe']) ? '<object type="image/svg+xml" data="' . $settings['images_url'] . '/forum_logo_default.svg" style="pointer-events: none;">' . $context['forum_name_html_safe'] . '</object>' : '<img src="' . $context['header_logo_url_html_safe'] . '" alt="' . $context['forum_name_html_safe'] . '">', '</a>
		</h1>';
	...
	if ($context['user']['is_logged'])
		echo '
						<ul class="unread_links">
							...
							<li>
								<a href="', $scripturl, '?action=recent" title="', $txt['recent_view'], '">', $txt['recent_posts'], '</a>
							</li>
						</ul>';
	...
	echo '
				<a class="mobile_user_menu">
					<span class="menu_icon">
						<object type="image/svg+xml" data="', $settings['images_url'], '/icons/hamburger.svg"></object>
					</span>
